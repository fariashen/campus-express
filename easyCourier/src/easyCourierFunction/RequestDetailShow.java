...
public class RequestDetailShow extends Activity {
	...
	// 网络请求类
	private HttpPostRequest RDSHttpPostRequest;
	...
	public static Handler handler_RequestDetailShow = new Handler() {
		...
		public void handleMessage(android.os.Message msg) {
			...
			case 2:
				// 接单失败 提示信息
				Toast.makeText(requestDetailShowContext, "接单失败,该单已被接！",
			...
			case 1:
				// 接单成功 跳转到快递请求的完整信息界面
				handleIntent = new Intent(requestDetailShowContext,
						ShowRequestAllInfo.class);
				
				mBundle.putBoolean("isCheckUpLoaded", false);
				mBundle.putBoolean("isCheckAccepted", false);
				
				handleIntent.putExtra("ItemAllInfo", mBundle);
				
				Toast.makeText(requestDetailShowContext, "接单成功！",
						Toast.LENGTH_LONG).show();
			...
			case 0:
				// 接单失败 提示信息
				Toast.makeText(requestDetailShowContext, "接单失败,系统繁忙！",
						Toast.LENGTH_LONG).show();
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		bt_acceptRequest.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				// TODO
				RDSHttpPostRequest = new AcceptRequestHttpPost();
